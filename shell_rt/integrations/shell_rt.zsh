...
if [[ -z ${SHELL_RT_DATA_ROOT-} ]]; then
  if [[ -f "$SHELL_RT_ROOT/shell_next_cmd_lstm.py" ]]; then
    typeset -g SHELL_RT_DATA_ROOT="$SHELL_RT_ROOT"
  else
    typeset -g SHELL_RT_DATA_ROOT="$PWD"
  fi
fi

typeset -g SHELL_RT_MODEL="${SHELL_RT_MODEL:-$SHELL_RT_DATA_ROOT/model/checkpoint.pt}"
typeset -g SHELL_RT_FEEDBACK_STORE="${SHELL_RT_FEEDBACK_STORE:-$SHELL_RT_DATA_ROOT/feedback/events.jsonl}"
typeset -g SHELL_RT_ONLINE_LEARNING="${SHELL_RT_ONLINE_LEARNING:-0}"
typeset -g SHELL_RT_ONLINE_STATE="${SHELL_RT_ONLINE_STATE:-$SHELL_RT_DATA_ROOT/feedback/online_state.json}"
...
typeset -ga SHELL_RT_CLI_BASE

function shell_rt_cli_base() {
  emulate -L zsh
  setopt no_aliases

  if [[ -n ${SHELL_RT_COMMAND-} ]]; then
    SHELL_RT_CLI_BASE=("${(@z)SHELL_RT_COMMAND}")
...
function shell_rt_accept_suggestion() {
  ...
  if [[ "$SHELL_RT_ONLINE_LEARNING" == "1" ]]; then
    (
      ...
      "${SHELL_RT_CLI_BASE[@]}" online-learn \
        --model "$SHELL_RT_MODEL" \
        --store "$SHELL_RT_FEEDBACK_STORE" \
